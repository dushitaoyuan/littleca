...
public class HmacSignUtil {
    ...
    public static com.taoyuanx.auth.sign.impl.hmac.Mac getHmac(Digest digest, byte[] key) {
        KeyParameter keyParameter = new KeyParameter(key);
        HMac hmac = new HMac(new SM3Digest());
        hmac.init(keyParameter);

        return new com.taoyuanx.auth.sign.impl.hmac.Mac() {
            @Override
    ...
    public static com.taoyuanx.auth.sign.impl.hmac.Mac getHmac(HMacAlgorithms macAlgorithms, byte[] key) {
        try {
